...
del mismo*/
/*
SELECT ciudad.nombre as 'ciudad', pais.nombre as 'pais', usuario.nombre, usuario.fecha_nacimiento FROM comentarios
INNER JOIN usuario ON usuario.id = comentarios.usuario_id
INNER JOIN ciudad ON ciudad.id = usuario.ciudad_id
INNER JOIN pais ON pais.id = ciudad.pais_id
WHERE usuario.fecha_nacimiento between year(curdate()-(1978-01-01) AND (1986-01-01; */
