...
-- CANTIDAD DE REGISTROS DEVUELTOS 50
...
-- 1 REGISTRO DEVUELTO = 40 RECURSOS UTILIZADOS DURANTE EL AÑO 2022
...
select count(ar.estadoart) as CantArticulo,au.usuario,au.email,pa.nombre as Pais,ar.estadoart
from autores au 
...
-- SE DEVUELVEN 8 REGISTROS 

/*consigna 15*/
SELECT apellido, nombre, TIMESTAMPDIFF(YEAR,fecha_nacimiento,CURDATE()) AS edad,
CASE
WHEN TIMESTAMPDIFF(YEAR,fecha_nacimiento,CURDATE()) < 36 THEN 'junior'
...
SELECT count(comentarios.cometario) as 'cantidadComentarios', usuario.nombre, usuario.apellido, pais.nombre FROM comentarios
INNER JOIN usuario ON usuario.id = comentarios.usuario_id
...
INNER JOIN articulos ON articulos.id = recursos.articulos_id
INNER JOIN subcategorias ON subcategorias.id = articulos.subcategorias_id
WHERE tiposrecursos.id = 1  AND recursos.ruta LIKE '%.mp3';
...
SELECT year(curdate()) - year(fecha_nacimiento) Edad , u.nombre, u.apellido,
ciudad.nombre , pais.nombre ,c.Cometario
from usuario u
inner join comentarios c on c.usuario_id = u.id
left join ciudad on ciudad.id = u.ciudad_id
left join pais on pais.id = ciudad.pais_id
where year(curdate()) - year(fecha_nacimiento) between 36 and 40
;
